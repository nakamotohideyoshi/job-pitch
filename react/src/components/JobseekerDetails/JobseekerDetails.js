...
const JobseekerDetails = ({ jobseekerData, application, actions, defaultTab }) => {
  const jobseeker = jobseekerData || application.job_seeker;
  const avatar = helper.getAvatar(jobseeker);
  const fullName = helper.getFullName(jobseeker);
  const age = jobseeker.age_public && jobseeker.age;
  const sex = jobseeker.sex_public && helper.getNameByID(DATA.sexes, jobseeker.sex);
...
JobseekerDetails.propTypes = {
  jobseekerData: PropTypes.object,
  application: PropTypes.object,
  actions: PropTypes.node,
  defaultTab: PropTypes.string
};

JobseekerDetails.defaultProps = {
  jobseekerData: null,
  application: null,
  actions: null,
  defaultTab: null
