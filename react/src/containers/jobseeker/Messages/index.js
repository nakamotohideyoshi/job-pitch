...
class Messages extends React.Component {
  ...
  renderHeader = selectedApp => {
    const { job_data } = selectedApp;
  ...
  render() {
    ...
    const { openJobDetails } = this.state;
    ...
    return (
      <StyledLayout>
        ...
        <Layout.Content>
          {selectedApp && (
            <Fragment>
              {this.renderHeader(selectedApp)}
              <div className="content">
                <MessageThread userRole="JOB_SEEKER" application={selectedApp} onSend={this.onSend} />
              </div>
            </Fragment>
          )}
        </Layout.Content>
