...
import { PopupProgress, Icons, VideoRecorder, VideoPlayer, LinkButton } from 'components';
...
class PitchRecord extends React.Component {
  ...
  openRecorder = () => {
    if (navigator.userAgent.indexOf('iPhone') !== -1) {
      ...
      this.setState({ visibleRecorder: true });
    }
  };
  ...
  closeRecorder = (url, data) => {
    ...
    this.setState({ pitchUrl, pitchData, visibleRecorder: false });
  };
  ...
  render() {
    const { pitchUrl, pitchData, loading, visibleRecorder, poster } = this.state;
    ...
    return (
      <Wrapper>
        ...
        {visibleRecorder && <VideoRecorder onClose={this.closeRecorder} />}
