...
export function getApplicationStatusByName(name) {
  return ApiClient.applicationStatuses.filter(item => item.name === name)[0];
}

export function getSexById(id) {
  return ApiClient.sexes.filter(item => item.id === id)[0];
}

export function getJobStatus(job) {
  const i = _.findIndex(ApiClient.jobStatuses, { id: job.status });
  return ApiClient.jobStatuses[i].name;
}
