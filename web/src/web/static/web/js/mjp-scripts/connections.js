						...
						if(data.length != 0){
							for (var key in data) {
									  ...
									  if(businessUser == 1){
										  ...
										  //if deleted
										  if(obj.status == 3){
											$('#list-table tbody').append('<tr data-application-id="'+obj.id+'" class="application-list inactive" id="application-list-'+obj.id+'"><td>'+obj.job_data.title+'</td><td>'+messageShort+'...<br><span class="message-time-date">'+date.getHours()+':'+minutesTwoDigitsWithLeadingZero+' '+date.getDate()+'/'+date.getMonth()+'/'+date.getFullYear()+'</span></td></tr>');
										  }else{
