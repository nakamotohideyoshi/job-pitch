					...
					$.get("/api/jobs/", {
						...
						for (var key in data) {
							...
							if (imageThumb != '') {
								...
								$('#list-table tbody').append('<tr data-job-id="' + obj.id + '" class="job-list" id="job-list-' + obj.id + '"><td> <img width="150px" src="'+CONST.PATH.NOIMAGE+'"></td><td onclick="goToJob(' + obj.id + ');">' + obj.title + '</td><td onclick="goToJob(' + obj.id + ');" class="jobTableDescription2">' + obj.description + '</td><td onclick="goToJob(' + obj.id + ');">' + obj.location_data.place_name + '</td></tr>');
							}
						}
						if (data.length == 0) {
							$('#jobListMessages').show();
